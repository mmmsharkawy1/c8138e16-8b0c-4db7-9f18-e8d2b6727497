...
- [x] 38 RLS policies implemented (rls_policies.sql)
- [x] 15 core functions ready (core_functions.sql + bundle_functions.sql)
...
wholesale-platform/
├── apps/
│   ├── dashboard/          # Next.js (Merchant Management)
│   ├── pos/                # Next.js PWA + Electron (Hardware)
│   └── storefront/         # Next.js (B2B Portal)
├── packages/
...
## ☁️ Step 2: Supabase Configuration (Day 1-2)
...
```bash
supabase/migrations/
├── 001_core_schema.sql         # Core 22 tables
├── 002_seed_niche_templates.sql # 3 niche types
├── 003_saas_governance.sql     # Limit functions
├── 004_rls_policies.sql        # 38 security policies
├── 005_core_functions.sql      # 15 inventory/sales functions
├── 006_bundle_functions.sql    # sell_bundle()
├── 007_seed_subscription_plans.sql # Free/Silver/Gold plans
└── 008_test_inventory_concurrency.sql # Concurrency test suite
```
...
## 🔄 Step 3: PowerSync Configuration (Day 2-3)
...
## 🖥️ Step 4: Electron POS Wrapper (Day 3-4)
...
## 🧙 Step 5: Onboarding Wizard (Day 4-5)
...
const niches = [
  ...
  { type: 'auto_parts', name: 'Auto Parts', icon: '🚗' },
  { type: 'fmcg', name: 'FMCG & Food', icon: '🍎' },
];
  ...
  const handleComplete = async () => {
    await fetch('/api/tenant/settings', {
      ...
      body: JSON.stringify({ 
        niche_type: selected,
        preferences: {
          theme: { primary_color: color, logo_url: null }
        }
      })
    });
...
## 📦 Step 6: Bundle Management UI (Day 5-6)
...
- [ ] Branding & Theme Configuration complete
...
## 📊 Success Metrics (TAGER ERP KPIs)
